...
func RegisterAllUpdates() map[string][]DatabaseUpdateFunc {
	...
	allUpdates = RegisterDatabaseUpdate(allUpdates, DatabaseUpdate_update0019_update0019testdata)
...
func InitDatabase(cfg *config.Config) (*sql.DB, error) {
	...
	for installedSomeUpdate { // while
		...
		for _, installed_update_id := range installedUpdates {
			...
			slog.Debug("Installed update: " + installed_update_id)
		}
	}
	// successfully
...
func ParseNameFuncUpdate(pc uintptr, file string, line int, ok bool) (string, string) {
	...
	// remove prefix ctf01d/internal/database.DatabaseUpdate_update0000_update0001, leave only the name of func: DatabaseUpdate_update0000_update0001
	functionName = strings.Split(functionName, "psql.")[1]
